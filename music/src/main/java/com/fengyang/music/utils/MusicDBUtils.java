 ...
 */
public class MusicDBUtils extends SQLiteOpenHelper {

	public MusicDBUtils(Context context){
	...
	public synchronized void addMusic(Music music){
		if(! isLiked(music)){
			SQLiteDatabase db = getWritableDatabase();
			Cursor cursor = db.rawQuery("select * from t_like where title = ?",new String[]{music.getTitle()});
